...
void print_stat_menu( PLAYER_DATA *ch )
{
    ...
    if ( ch->exp_level > 0 && PC(ch,stat_points) == 0 ) {
        ...
        DC(ch->desc) = CON_HERO_RECON;
        return;
    }
   
    if ( ch->exp_level == 0 ) cmd_help( ch, "NANNY_STAT_MENU_HEADER" ); 
    else 
...
void stat_menu( PLAYER_DATA *ch, char *argument )
{
    ...
    {
    case 's':
        cmd_help( ch, "NANNY_STRENGTH" );
        write_to_buffer( d, "Do you wish to increase or decrease this stat? ", 0 );
        DC(d) = CON_STAT_STR;
      break;
    case 'i':
        cmd_help( ch, "NANNY_INTELLIGENCE" );
        write_to_buffer( d, "Do you wish to increase or decrease this stat? ", 0 );
        DC(d) = CON_STAT_INT;
      break;
    case 'w':
        cmd_help( ch, "NANNY_WISDOM" );
        write_to_buffer( d, "Do you wish to increase or decrease this stat? ", 0 );
        DC(d) = CON_STAT_WIS;
      break;
    case 'd':
        cmd_help( ch, "NANNY_DEXTERITY" );
        write_to_buffer( d, "Do you wish to increase or decrease this stat? ", 0 );
        DC(d) = CON_STAT_DEX;
      break;
    case 'c':
        cmd_help( ch, "NANNY_CONSTITUTION" );
        write_to_buffer( d, "Do you wish to increase or decrease this stat? ", 0 );
        DC(d) = CON_STAT_CON;
      break;
    case 'a':
...
/***/   DC(d) = CON_STAT_SIZE;
   ...
   default:
    ...
    cmd_help( ch, "WELCOME" );
    DC(d) = CON_READ_MOTD;
          } else {  DC(d) = CON_HERO_RECON_ABORT;  
             ...
             case 'i':
                {
                    ...
                    if (((DC(d) == CON_STAT_STR && ch->perm_str >= 25)
                      || (DC(d) == CON_STAT_INT && ch->perm_int >= 25)
                      || (DC(d) == CON_STAT_WIS && ch->perm_wis >= 25)
                      || (DC(d) == CON_STAT_DEX && ch->perm_dex >= 25)
                      || (DC(d) == CON_STAT_CON && ch->perm_con >= 25)) && (ch->exp_level == 0) )
                    {
...
void actor_gen( PLAYER_DATA *ch, char *argument )
{
    ...
    case CON_CONFIRM_NEW_PASSWORD:
        ...
        cmd_help( ch, "NANNY_INTRO" );
	write_to_buffer( d, "Press enter to continue: ", 0 );
        DC(d) = CON_SHOW_INTRO;
	   break;

	case CON_SHOW_INTRO:
        cmd_help( ch, "NANNY_NAME" );
   ...
   case CON_CHAR_GEN_EMAIL:
        ...
        DC(d) = CON_CHAR_GEN_BUILDER;
        break;

    case CON_CHAR_GEN_BUILDER:
        if ( UPPER(*argument) == 'Y' )
        {
...
//            send_to_actor( "Ok.\n\r", ch );
        ...
        DC(d) = CON_CHAR_GEN_COLOR;
        }

        break;

    case CON_CHAR_GEN_WRITER:
        ...
        if ( UPPER(*argument) == 'Y' ) {
           ...
           DC(d) = CON_CHAR_GEN_APPLY_BUILDER;
...
#if defined(APPLY_BUILDER) 
     case CON_CHAR_GEN_APPLY_BUILDER:
           if ( d->pString == NULL  ) {
           actor_to_scene( ch, get_scene_index( SCENE_VNUM_APPLY ) );
   ...
   case CON_CHAR_GEN_RACE:
        {
              ...
              DC(d) = CON_CHAR_GEN_RACE_CONFIRM;
        }
       break; 
   case CON_CHAR_GEN_RACE_CONFIRM:
        {
              switch ( *argument ) {
                case 'n': case 'N':
                   cmd_help( ch, "RACES" );
...
//        write_to_buffer( d, echo_on_str, 0 );
    ...
    while ( DC(d) > CON_PLAYING && !isalnum(*argument) && *argument != '\n' && *argument != '\r' && *argument != '\0'  ) { argument++; }
    switch ( DC(d) )
    {
...
void nanny_check( CONNECTION_DATA *d )
{
