...
*
* If you use this code, enjoy it, have wtf_logf fixes or simply want to
 ...
 * 	One line of 80 charflagers can take up a lot of space if every 
 * 	spot is used and has an extra 4-8 colour code charflagers in it.
...
/*		wiznet ("New map_param created", NULL, NULL, WIZ_MEM, 0, 0);*/
	...
	{ // Can call function with NULL (if used externally)
		sprintf (mp->desc, "%s", ch->in_scene->description);
		clean_map_string(ch);
	}
	return;
}
...
// Returns the charflager of the link
...
// Returns the charflager of the scene.
// Change as you like
	...
	for (y = y_from; y <= y_to; y++)		
	{
		for (x = x_from; x <= x_to; x++)
		{
			if (map[x][y] != -1) // Is there something here?
			{
				...
				else if (xx == x && yy == y) // This is where we started.  Make sure to mark it with a special charflager
				{
					if (mp->col == TRUE) {
                                        if ( HAS_ANSI(ch) ) 
